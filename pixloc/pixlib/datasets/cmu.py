...
class _Dataset(torch.utils.data.Dataset):
        ...
        for slice_ in tqdm(self.slices):
            ...
            if self.conf.two_view:
                ...
                if self.conf.min_baseline:
                    pairs &= (info['query_to_ref_distance_matrix'] > self.conf.min_baseline)
                    
                if self.conf.max_baseline:
                    pairs &= (info['query_to_ref_distance_matrix'] < self.conf.max_baseline)
    ...
    def _read_view(self, slice_, idx, common_p3D_idx, is_reference=False):
        ...
        if is_reference:
            ...
            if num_diff < 0:
                ...
                # np.delete():np.arange(len(p3D))からobs番目を削除。axis指定が無ければ1次元に平坦化された上でobsで指定したインデックスの要素が削除
    ...
    def __getitem__(self, idx):
        if self.conf.two_view:
            ...
            obs_q = self.info[slice_]['p3D_observed'][self.info[slice_]['query_closest_indices'][idx_q]]
