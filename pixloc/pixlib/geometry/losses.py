...
def scaled_loss(x, fn, a):
    ...
    a2 = a**2 # 0.1**2
    loss, loss_d1, loss_d2 = fn(x/a2) # fn:barron_loss(y, y.new_tensor(0))
...
def huber_loss(x):
    ...
    # torch.finfo(torch.float).eps:表現可能な最小の数。
    isx = torch.max(sx.new_tensor(torch.finfo(torch.float).eps), 1/sx)
    
    # torch.where(condition, condition is true:x, condition is false:y)
    ...
    # 二階導関数:ある量の変化率そのものがどのように変化しているかを測定する
    # x <= 1 : 0
    # 1 <  x : -1/(x**1/2) / (x*2)
...
def barron_loss(x, alpha, derivatives: bool = True, eps: float = 1e-7):
    ...
    loss_two = x
    loss_zero = 2 * torch.log1p(torch.clamp(0.5*x, max=33e37)) # 33e37以下
