...
def training(rank, conf, output_dir, args):
    ...
    while epoch < conf.train.epochs and not stop:
        ...
        if epoch > 0 and conf.train.dataset_callback_fn:
            getattr(train_loader.dataset, conf.train.dataset_callback_fn)(conf.train.seed + epoch)
            ...
            if do_backward:
                ...
                if conf.train.get('clip_grad', None):
                    if it % conf.train.log_every_iter == 0:
                        grads = [p.grad.data.abs().reshape(-1) for p in all_params if p.grad is not None]
                        ratio = (torch.cat(grads, 0) > conf.train.clip_grad)
                        ratio = ratio.float().mean().item()*100
                        
                        if ratio > 25:
                            logger.warning(f'More than {ratio:.1f}% of the parameters are larger than the clip value.')
                            
                        del grads, ratio
                        
                    torch.nn.utils.clip_grad_value_(all_params, conf.train.clip_grad)
        ...
        if rank == 0:
            ...
            if results[conf.train.best_key] < best_eval:
                ...
                logger.info(f'New best checkpoint: {conf.train.best_key}={best_eval}')
